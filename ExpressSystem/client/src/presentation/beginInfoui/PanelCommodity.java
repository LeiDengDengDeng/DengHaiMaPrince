...
public class PanelCommodity extends SubPanel {
	...
	public ArrayList<Object> getVO() {
		...
		for (int i = 0; i < this.getJComBox().size(); i++) {
			...
			double alarmScale = Double.parseDouble(this.getArrayList().get(i)[3].getText());// 警戒比例
			int rowNum = Integer.parseInt(this.getArrayList().get(i)[1].getText());// 仓库排数
			int shelfNum = Integer.parseInt(this.getArrayList().get(i)[0].getText());// 仓库架数
			int seatNum = Integer.parseInt(this.getArrayList().get(i)[2].getText());// 仓库位数
