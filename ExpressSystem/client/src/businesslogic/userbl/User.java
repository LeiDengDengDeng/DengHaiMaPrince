...
//					password, password);
			try {
				...
				log.generateLog("修改密码", String.valueOf(userPO.getPersonalID()));
			} catch (RemoteException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	public boolean initial(ArrayList<InitUserVO> User) {
		...
		try {
			...
			log.generateLog("初始化账号", String.valueOf(userPOs.size()));
		} catch (RemoteException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
...
//		System.out.println(user.getPersonalInfo(100000).getSalary().getBasic());
